...
export default function DocumentsIndex({ items }: { items: Row[] }) {
    const remove = (row: Row) => {
        if (confirm(`Удалить «${row.title}»?`)) {
            router.delete(`/admin/documents/${row.id}`, { preserveScroll: true });
        }
    };
        ...
        <>
            ...
            <div className="flex items-center justify-between gap-4">
                <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">Реестр документов портала.</p>
                </div>
                <Button asChild>
                    ...
                    <tbody className="divide-y">
                        {items.map((row) => (
                            <tr key={row.id} className="hover:bg-muted/30">
                                ...
                                <td className="p-3 text-muted-foreground">{row.number ?? '—'}</td>
                                <td className="p-3 text-muted-foreground">{row.document_date ?? '—'}</td>
                                <td className="p-3">
                                    <div className="flex justify-end gap-1">
                                        <Button asChild size="icon" variant="ghost">
                                            <Link href={`/admin/documents/${row.id}/edit`} aria-label="Редактировать">
